...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		...
		// 인터페이스 타입을 사용하는 이유?  다형성 사용(앞의 타입이 상위 타입이어야 함)
		// List 인터페이스의 다른 구현 클래스를 사용하게 되더라도
		// 코드를 수정하지 않고 새로운 기능을 추가할 수 있음
		
		// 제네릭 : 자바에서 클래스나 메소드가 사용할 타입을 
		// 외부에서 파라미터처럼 받아서 사용할 수 있게 해주는 방법
		// 타입 파라미터<> : 제네릭을 사영할 때 전달하는 타입
...
//		list.add(7, "TEST");  // IndexOutOfBoundsException 발생 (사이즈보다 인덱스가 큰 경우)
		...
		list.remove(2);
		list.remove("MyBatis");  //Object로 삭제
		System.out.println(list);
		...
		// 객체 검색
		// contains() : 값이 있는지 없는지 boolean 으로
		// indexOf() : 값이 어느 인덱스 위치에 있는지
		if (!list.isEmpty()) {  // 컬렉션이 비어 있는지 조사
			// 주어진 객체가 저장되어 있는지 조사
			System.out.println("JDBC 있니? " + list.contains("JDBC"));  // boolean 값 출력
			System.out.println("어디에 있니? " + list.indexOf("JDBC"));  // 인덱스 값 출력
			System.out.println("Database 있니? " + list.contains("Database"));  // boolean 값 출력
			System.out.println("어디에 있니? " + list.indexOf("Database"));  // 인덱스 값 출력
		}
		System.out.println();
		...
		// 지정된 모든 객체 삭제
		list.clear();
		System.out.println(list.isEmpty());  // 비어 있으면 true 출력됨
