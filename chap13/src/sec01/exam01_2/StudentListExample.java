...
public class StudentListExample {
	...
	public static void main(String[] args) {
		...
		studentList.add(new Student("고은빈", 30));
		studentList.add(new Student("김재현", 25));
		studentList.add(new Student("이수진", 40));
		
		// 객체 검색
		// 사용자 정의 객체를 사용할 경우 indexOf(), contains() 가 해당 객체 존재 여부를 판단하는 기준은 equals()
		// equals() 메소드를 재정의 해야 동등 객체로 판단
		if (studentList.indexOf(new Student("고은빈", 30)) != -1) {
			System.out.println("indexOf: 있다.");
		} else {
			System.out.println("indexOf: 없다.");
		}
		
		if (studentList.contains(new Student("고은빈", 30))) {
